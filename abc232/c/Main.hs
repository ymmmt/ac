...
edges :: Int -> [(Int, Int)]
edges n = map tuple . choices 2 $ [1..n]

permEdges :: Int -> [[(Int, Int)]]
permEdges n = map (map tuple . choices 2) $ permutations [1..n]

solve :: Int -> [[Int]] -> [[Int]] -> Bool
solve n abs cds = any match $ permEdges n
  where mkG uvs = accumArray (||) False ((1, 1), (n, n)) $ zip (concatMap tuples uvs) (repeat True)
        g = mkG abs
        h = mkG cds
        es = edges n
        match permEs = all id $ zipWith (==) (map (g!) es) (map (h!) permEs)
