...
(defun add (cum-counts start end count)
  (incf (aref cum-counts start) count)
  (decf (aref cum-counts end) count)
  cum-counts)

(defun solve (n d a xhs)
  (let ((xhs (coerce-vector (sort xhs #'< :key #'car)))
        (cum-counts (make-fixnum-array (1+ n)))
        (ans 0))
    (map nil
         (dlambda ((x . h) i)
           (when (plusp i)
             (incf (aref cum-counts i)
                   (aref cum-counts (1- i))))
           (let* ((rest (max 0 (- h (* a (aref cum-counts i)))))
                  (c (ceiling rest a)))
             (when (plusp c)
               (let ((j (bisect-right xhs (+ (2* d) x) :key #'car)))
                 (add cum-counts i j c)
                 (incf ans c)))))
         xhs (range n))
    ans))              
...
(defun main ()
  (readlet (n d a)
    (let ((xhs (read-conses n)))
      (println (solve n d a xhs)))))
