...
(defun edges (n-vertices)
  (let ((ijs (collecting* ((i 0 n-vertices)
                           (j (1+ i) n-vertices))
               (cons i j))))
    (nconc ijs
           (mapcar (dlambda ((i . j))
...
(defun solve (n xyzs)
  (let ((xyzs (coerce-vector xyzs)))
    (with-memos ((cost (i j)
                   (dbind (xi yi zi) (svref xyzs i)
                     (dbind (xj yj zj) (svref xyzs j)
                       (+ (manhattan xi yi xj yj)
                          (max 0 (- zj zi))))))
                 (dp (i unseen)
