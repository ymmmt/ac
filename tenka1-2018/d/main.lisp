...
(defun solve (n)
  (let ((k (/ (1+ (sqrt (1+ (* 8 n))))
              2)))
    (when (= k (floor k))
      (let* ((set-add-alist (alist (flatten (combinations (range 1 (1+ k)) 2))
                                   (flatten (mapcan (curry #'repeat 2) (range 1 (1+ n))))))
             (set (list-accum-ht set-add-alist)))
        (values (floor k) set)))))

(defun main ()
  (readlet (n)
    (mvbind (k set) (solve n)
      (if k
          (bulk-stdout
            (println "Yes")
            (println k)
