...
;; https://atcoder.jp/contests/abc270/editorial/4847
(defun baby-step-giant-step (f^m f^-1 x y m)
  (let ((invs (list-accum-ht (zip-with-index (iterate (1- m) f^-1 y))))
        (powers (iterate (1+ m) f^m x)))
    (mvbind (js i) (position-if* (mapper invs) powers)
      (if i
          (+ (* i m) (car js))         ; 存在するとすればこの値はp未満
          -1))))

(defun f^m (p m a b)
  (mvbind (a* b*)
      (transit (1- m) (lambda (prev-a prev-b)
                        (values (mod (* prev-a a) p)
                                (mod (+ (* prev-b a) b) p)))
               a b)
    (lambda (x)
      (mod (+ (* a* x) b*)
...
(defun f^-1 (p a b)
  (let ((a^-1 (mod-inverse a p)))
    (lambda (y)
      (mod (* (- y b) a^-1)
           p))))

(defun answer (p a b s g)
  (cond ((= s g) 0)
        ((zerop a)
         (if (= b g) 1 -1))
        (t
         (let ((m (isqrt p)))
           (baby-step-giant-step (f^m p m a b) (f^-1 p a b)
                                 s g m)))))

(defun solve (qs)
  (mapcar (applier #'answer) qs))
