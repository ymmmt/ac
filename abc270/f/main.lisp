...
(defun solve (n xs ys abs ws)
  (let* ((airport-index n)
         (port-index (1+ n))
         (es-airport (mapcar $(cons % airport-index)
                             (range n)))
         (es-port (mapcar $(cons % port-index)
                          (range n)))
         (w (ht abs ws :test #'equal))
         (w-airport (ht es-airport xs :test #'equal))
         (w-port (ht es-port ys :test #'equal)))
    (labels ((weight (edge)
               (assert (< (car edge) (cdr edge)))
               (gethash-union edge (w w-airport w-port) +inf+))
             (mst-cost (es n-vertices cc-size)
               (mvbind (_ cost vs) (mst-kruskal n-vertices es #'weight)
                 (if (= (ds-size vs 0) cc-size)
                     cost
                     +inf+))))
      (minimize ((es n-vertices cc-size) (list `(,abs ,n ,n)
                                               `(,(append abs es-airport) ,(+ n 1) ,(+ n 1))
                                               `(,(append abs es-port) ,(+ n 2) ,(+ n 1)) ; mst-kruskarlの仕様に合わせた汚いハック
                                               `(,(append abs es-airport es-port) ,(+ n 2) ,(+ n 2))))
        (mst-cost es n-vertices cc-size)))))
...
(defun main ()
  (readlet (n m)
    (let ((xs (readlist))
          (ys (readlist)))
      (mvbind (abs ws) (read-weighted-edges m :directed t)
        (println (solve n xs ys abs ws))))))
