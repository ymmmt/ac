...
(defun solve (n xs ys abzs)
  (let ((airport-index n)
        (port-index (1+ n)))
    (let ((ws (ht (mapcar (dlambda ((a b z))
                            (declare (ignore z))
                            ;; (< a b)
                            (cons a b))
                          abzs)
                  (mapcar #'third abzs)
                  :test #'equal))
          (ws-airport (ht (mapcar $(cons % airport-index)
                                  (range n))
                          xs
                          :test #'equal))
          (ws-port (ht (mapcar $(cons % port-index)
                               (range n))
                       ys
                       :test #'equal)))
      (labels ((weight (u v)
                 (assert (/= u v))
                 (sortf < u v)
                 (cond ((= v airport-index)
                        (gethash (cons u v) ws-airport))
                       ((= v port-index)
                        (gethash (cons u v) ws-port))
                       (t
                        (gethash (cons u v) ws +inf+))))
               (mst-cost (es n-vertices cc-size)
                 (mvbind (_ cost vs) (mst-kruskal n-vertices es #'weight)
                   (if (= (ds-size vs 0) cc-size)
                       cost
                       +inf+))))
        (let* ((es1 (mapcan (dlambda ((a b z))
                              (list (cons a b)
                                    (cons b a)))
                            abzs))
               (es-airport (mapcan (lambda (v)
                                     (list (cons v airport-index)
                                           (cons airport-index v)))
                                   (range n)))
               (es-port (mapcan (lambda (v)
                                  (list (cons v port-index)
                                        (cons port-index v)))
                                (range n)))
               (es2 (append es1 es-airport))
               (es3 (append es1 es-port))
               (es4 (append es1 es-airport es-port)))
          (minimize ((es n-vertices cc-size) (list `(,es1 ,n ,n)
                                                   `(,es2 ,(+ n 1) ,(+ n 1))
                                                   `(,es3 ,(+ n 2) ,(+ n 1)) ; mst-kruskarlの仕様に合わせた汚いハック
                                                   `(,es4 ,(+ n 2) ,(+ n 2))))
            (mst-cost es n-vertices cc-size)))))))
