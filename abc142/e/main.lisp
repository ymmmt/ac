...
(defun vectorize (list)
  (pad (coerce-vector list)))

(defun solve (n m acs)
  (let ((a (vectorize (mapcar #'car acs)))
        (c (vectorize (mapcar @($(apply #'bitset) #'cdr) acs))))
    (with-cache (:array ((expt 2 n) (1+ m)) :initial-element -1)
      (labels ((dp (bs i)
                 (cond ((bs-empty-p bs) 0)
                       ((zerop i) +inf+)
                       (t (min (dp bs (1- i))
                               (+ (dp (bs-difference bs (aref c i)) (1- i))
                                  (aref a i)))))))
        (let ((ans (dp (bs-universal n) m)))
          (if (= ans +inf+)
              -1
              ans))))))
...
(defun main ()
  (readlet (n m)
    (let ((acs (collect m
                 (readlet (a b)
                   (declare (ignore b))
                   (let ((cs (mapcar #'1- (readlist))))
                     (cons a cs))))))
      (println (solve n m acs)))))
