...
(defun border (max cum k)
  (if (zerop k)
      (1+ max)
      (let* ((l (length cum))
             (i (bisect-right (lambda (i)
                                (cdr (aref cum i)))
                              k
                              :end l)))
        (if (= i l)
...
(defun solve (n ss q ks)
  (declare (ignore n))
  (let ((max (reduce #'max ss))
        (cum (-> (sort ss #'>)
                 (remove 0 %)
                 (multiset #'=)
                 (scanl (dlambda ((score . count) (si . ci))
                          (cons si (+ count ci)))
                        (cons 0 0)
                        %)
                 cdr
    ...
    (if (zerop (length cum))
        (repeat q 0)
        (mapcar (curry #'border max cum)
                ks))))

(defun main ()
  (readlet (n)
    (let ((ss (collect n (read-fixnum))))
      (readlet (q)
        (let ((ks (collect q (read-fixnum))))
          (bulk-stdout
            (printlns (solve n ss q ks))))))))
