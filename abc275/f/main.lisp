...
(defun solve (n m a)
  (let ((inf (1+ n)))
    (with-caches ((:array (n (1+ m)) :element-type 'fixnum :initial-element -1)
                  (:array (n (1+ m)) :element-type 'fixnum :initial-element -1))
      (labels ((dp1 (i sum)
                 (let ((ai (aref a i)))
                   (cond ((zerop i)
                          (if (= sum ai) 0 inf))
                         ((> ai sum) inf)
                         (t             ; (and (plusp i) (<= ai sum))
                          (min (dp1 (1- i) (- sum ai))
                               (dp2 (1- i) (- sum ai)))))))
               (dp2 (i sum)
                 (cond ((zerop i)
