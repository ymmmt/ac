...
(defun solve (n m k)
  (with-cache (:array ((1+ n) (1+ k)) :element-type '(integer -1 998244352) :initial-element -1)
    (labels ((prob (pos count)
               (cond ((= pos n) 1)
                     ((= count k) 0)
                     (t
                      (rational->mod
                       (/ (sum (delta 1 (1+ m))
                            (prob (fold-back pos n delta)
                                  (1+ count)))
                          m))))))
      (prob 0 0))))
