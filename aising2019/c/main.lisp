...
(defun diff-color-p (s i j k l)
  (char/= (aref s i j)
          (aref s k l)))

(defun row-edges (h w s i)
  (map-adjacents (lambda (j1 j2)
                   (when (diff-color-p s i j1 i j2)
                     (cons (row-major-index i j1 w)
                           (row-major-index i j2 w))))
                 (range w)))

(defun col-edges (h w s j)
  (map-adjacents (lambda (i1 i2)
                   (when (diff-color-p s i1 j i2 j)
                     (cons (row-major-index i1 j w)
                           (row-major-index i2 j w))))
                 (range h)))

(defun whitep (s rmi)
  (char= (row-major-aref s rmi)
         #\.))
...
;; https://blog.hamayanhamayan.com/entry/2019/01/13/014815
(defun solve (h w s)
  (let* ((edges (delete nil
                        (nconc (mapcan (curry #'row-edges h w s)
                                       (range h))
                               (mapcan (curry #'col-edges h w s)
                                       (range w)))))
         (ds (alist->ds edges (* h w)))
         (roots (ds-roots-size>=2 ds))
         (g (make-graph (* h w) edges)))
    (let ((seen (make-bit-array (* h w))))
      (sum (u roots)
        (let* ((b (graph-count-if (curry #'blackp s)
                                  g
                                  u
                                  seen))
               (w (- (ds-size ds u) b)))
