...
(defun solve (n a)
  (if (every (lambda (i)
               (<= (svref a i) i))
             (range n))
      ;; 操作ごとに数列Xは狭義単調増加することに注意する。すなわち、
      ;; 操作を加えると、あるiについてXiは元のXiより大きく、
      ;; その他のjに関してXjは元と等しい。
      ;; (i+1)番目をa[i+1]に合わせた時点で、i番目は(a[i+1] - 1)になっている。
      ;; もしその数がaiに等しければ、i番目は操作の必要なし。
      ;; aiのほうが小さければXをAに合わせるのは不可能。
      ;; aiのほうが大きければ、ai個手前の数から順にインクリメントして
      ;; aiまで増やす必要があるので、必要な操作回数はai回。
      (reduce #'+ (range n)
              :key (lambda (i)
                     (cond ((= i (1- n))
                            (svref a i))
