...
(defun solve (n s)
  (labels ((paint (s i j)
           ...
           (zpaint (s i j)
             (when (>= i 0)
               (mapc (curry #'paint s i)
                     (range (1+ j))))
             (mapc (curry #'paint s (1+ i))
                   (range j n))
             s))
    (let ((cols (range n)))
      (mvfoldr (lambda (i acc s)
                 (aif (find-if (curry #'whitep s i) cols)
                      (values (1+ acc) (zpaint s (1- i) it))
                      (values acc s)))
               (range n)
               0 s))))
