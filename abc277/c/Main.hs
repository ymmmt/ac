...
solve :: Int -> [[Int]] -> Int
solve n abs = if Map.member 1 mp'
              then mp Map.! (maximum $ G.reachable g 1)
              else 1
  where
    mp           = coordComp 1 $ concat abs
    mp'          = Map.fromAscList $ zip (Map.elems mp) [1..]
    edges [a, b] = let a' = mp' Map.! a
                       b' = mp' Map.! b
                   in [(a', b'), (b', a')]
    g            = G.buildG (1, 2*n) $ concatMap edges abs
