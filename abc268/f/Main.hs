...
scoreX :: String -> X
scoreX = length . filter (== 'X')

scoreY :: String -> Y
scoreY = sum . map digitToInt . filter isDigit

scoreZ :: String -> Z
scoreZ = fst . foldl' step (0, 0)
  where step (acc, nx) c = if c == 'X'
                           then (acc, nx + 1)
                           else (acc + nx * digitToInt c, nx)

encode :: String -> (X, Y, Z)
encode s = (scoreX s, scoreY s, scoreZ s)

pri :: (X, Y, Z) -> (X, Y, Z) -> Ordering
pri (x1, y1, _) (x2, y2, _)
  | x2 == 0 || y1 * x2 < x1 * y2 = LT
  | y1 * x2 > x1 * y2            = GT
  | otherwise                    = EQ
...
solve n ss = fst $ foldl' step (0, 0) ss'
  where ss'                      = sortBy pri $ map encode ss
        step (acc, nx) (x, y, z) = (acc + nx * y + z, nx + x)
