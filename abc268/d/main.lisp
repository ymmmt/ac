...
(defun solve (n ss us)
  (if (= n 1)
      (remove-if (disjoin (mapper (make-hashset us :test 'equal))
                          (lambda (s)
                            (< (length s) 3)))
                 ss)
      (let ((k (- 16 (reduce #'+ ss :key #'length)))
            (us (make-hashset us :test 'equal)))
        (thereis (perm (permutations ss))
          (let (cands
                (s1 (car perm)))
            (labels ((rec (count acc ss)
                       (when (<= count 16)
                         (if (and (null ss)
                                  (<= 3 count))
                             (push (apply #'concatenate 'string
                                   ...
                                   cands)
                             (mapc-range (lambda (c)
                                           (rec (+ count c (length (car ss)))
                                                (list* (car ss) (make-string c :initial-element #\_)
                                                       acc)
                                                (cdr ss)))
                                         1 (1+ (min k (- 16 count (length (car ss))))))))))
              (rec (length s1) (list s1) (cdr perm))
              (remove-if (mapper us)
                         cands)))))))

(defun main ()
  (readlet (n m)
    (let ((ss (collect n (read-line)))
          (us (collect m (read-line))))
      (println (aif (solve n ss us)
                    (car it)
                    -1)))))
