...
(define lq (quantizer (eqler #\L) -1))
(define rq (quantizer (eqler #\R)))
(define uq (quantizer (eqler #\U) -1))
(define dq (quantizer (eqler #\D)))

(defun solve (h w n sr sc s1 s2)
  (declare (ignore n))
...
(defun main ()
  (readlet (h w n sr sc)
    (let ((s1 (read-line))
          (s2 (read-line)))
      (println (if (solve h w n (1- sr) (1- sc) s1 s2)
                   "NO"
                   "YES")))))
