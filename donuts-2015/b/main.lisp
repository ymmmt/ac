...
(defun solve (n m a combos)
  (declare (ignore m))
  (let ((a (pad a)))
    (labels ((bonus (member combo)
               (dbind (b is) combo
                 (if (>= (length (intersection member is))
                         3)
                     b
                     0))))
      (maximize (member (combinations (range 1 (1+ n)) 9))
        (+ (reduce #'+ member :key (arefer a))
           (sum (combo combos)
             (bonus member combo)))))))
...
(defun main ()
  (readlet (n m)
    (let ((a (readvector))
          (combos (collect m
                   (readlet (b c)
                     (let ((is (collect c (read-fixnum))))
                       (list b is))))))
      (println (solve n m a combos)))))
