...
(defun solve (as)
  (let* ((fss (mapcar #'prime-factors as))
         (lcm-prime-indices (accum-ht (mappend #'identity fss) 0 #'max))
         (lcm (reduce #'mod* (ht-keys lcm-prime-indices)
                      :key (lambda (p)
                             (mod-power p (gethash p lcm-prime-indices)
                                        +mod+)))))
    (reduce #'mod+ fss
            :key (lambda (fs)
                   (mod* lcm
