...
(defun solve (n edges cs)
  (let* ((g (make-graph n edges))
         (max (reduce #'max cs))
         (cs (cdr (sort cs #'>)))
         (vertex->c (make-fixnum-array n))
         (seen (make-bit-array n)))
    (setf (aref vertex->c 0) max)
    (setf (aref seen 0) 1)
    (nlet bfs ((q (snoc (make-queue) 0))
               (cs cs))
      (unless (queue-empty-p q)
        (let ((vs (filter (compose #'zerop (arefer seen))
                          (aref g (head q)))))
          (mapc (lambda (v c)
                  (setf (aref vertex->c v)
                        c)
                  (setf (aref seen v) 1))
                vs cs)
          (bfs (foldl #'snoc (tail q) vs)
               (drop (length vs) cs)))))
    (values (reduce #'+ cs)
            (mapcar (arefer vertex->c) (range n)))))

(defun main ()
  (readlet (n)
    (let ((edges (read-edges (1- n)))
          (cs (readlist)))
      (mvbind (m ds) (solve n edges cs)
        (println m)
        (join-print ds)))))
