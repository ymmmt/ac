...
(defvar *strings* '("dream" "dreamer" "erase" "eraser"))

(defun solve (s &optional (strings *strings*))
  (cond ((zerop (length s)) t)
        ((null strings) nil)
        ((suffixp s (car strings))
         (solve (slice s 0 (- (length s) (length (car strings))))))
        (t
         (solve s (cdr strings)))))
