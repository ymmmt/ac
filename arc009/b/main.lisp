...
(defun num->digits (num &optional (base 10))
  (assert (plusp num))
  (nlet rec ((num num) (acc nil) (length 0))
    (if (zerop num)
        (values acc length)
        (rec (floor num base)
             (cons (mod num base) acc)
             (1+ length)))))
...
(defun solve (b n a)
  (declare (ignore n))
  (let ((digit-map (make-hash-table)))
    (nlet rec ((i 0) (b b))
      (when b
        (setf (gethash (car b) digit-map) i)
        (rec (1+ i) (cdr b))))
    (sort a #'<
          :key (lambda (ai)
                 (digits->num (mapcar (mapper digit-map)
                                      (num->digits ai)))))))
                       
(defun main ()
  (let ((b (readlist)))
    (readlet (n)
      (let ((a (collect n (read-fixnum))))
        (printlns (solve b n a))))))
