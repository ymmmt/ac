...
(defun update (vs seen)
  (mapc (lambda (v)
          (setf (aref seen v) 1))
        vs)
  seen)

(defun no-edges-p (matrix vs)
  (or (length<= 1 vs)
      (every (compose #'zerop (applier (arefer matrix)))
             (combinations vs 2))))

(defun solve (n matrix)
  (let* ((es (lcomp ((i 0 n) (j 0 n))
                 (plusp (aref matrix i j))
               (cons i j)))
         (g (make-graph n es)))
    (labels ((rec (vs prev-vs seen k)
               (if (every #'plusp seen)
                   k
                   (mvbind (vs* seen-vs)
                       (partition (compose #'zerop (arefer seen))
                                  (delete-dups (mappend (arefer g) vs)))
                     (if (and (subsetp seen-vs prev-vs)
                              (no-edges-p matrix vs*))
                         (rec vs* vs (update vs* seen) (1+ k))
                         -1)))))
      (maximize (v 0 n)
        (let ((seen (make-bit-array n)))
          (setf (aref seen v) 1)
          (rec (list v) nil seen 1))))))

(defun main ()
  (readlet (n)
    (let ((g (read-matrix n n :read #'read-bit :element-type 'bit :read-newline t)))
      (println (solve n g)))))
