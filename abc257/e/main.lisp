...
;; https://blog.hamayanhamayan.com/entry/2022/06/25/233406
(defun solve (n c)
  (let ((c-min (reduce #'min c :start 1)))
    (nlet rec ((count (floor n c-min))
               (cost 0)
               (acc nil))
      (if (zerop count)
          (nreverse acc)
          (let ((i (position-if (lambda (ci)
                                  (<= (+ cost ci (* (1- count) c-min))
                                      n))
                                c :from-end t)))
            (rec (1- count)
                 (+ cost (svref c i))
