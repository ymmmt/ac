...
;; レベルdの子を持たない頂点の数をad、
;; 子を持つ親の数をpdで表す。
;; p[d-1] <= ad <= 2 * p[d-1] (d <- [1, n])
;; が成り立つ必要がある。この式を変形して
;; (ceiling (a[d+1] + p[d+1]) 2) <= pd <= a[d+1] + p[d+1] (d <- [0, n-1])
;; を得る。この第1辺を(p-min d)、第3辺を(p-max d)とする。
;; 更に、1個下のレベル(d-1)の子を持つ頂点の数をprev-level-nodesとすると、
;; (pd + ad) <= 2 * prev-level-nodes
;; である。よって
;; (p-min d) <= pd <= (min (2 * prev-level-nodes - ad)
;;                         (p-max d))
;; が常に成り立つ必要がある。
;; この制約を満たしつつ、できるだけ多くの頂点を
;; 各レベルで足していくと求めている最大値を得られる。
;; 制約を満たせなくなった時点で-1(不可能)を出力する。
(defun solve (n a)
  (when (zerop n)
    (return-from solve (if (= (svref a 0) 1)
                           1 -1)))
  (with-memos ((p-min (d)
                 (if (= d (1- n))
                     (ceiling (svref a n) 2)
