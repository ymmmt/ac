...
;; https://atcoder.jp/contests/abc272/submissions/35591264
(defun solve (n m as)
  (let* ((xiss1 (accum-array (flip #'cons) nil (1+ m)
                             (filter-map (dlambda ((a . i))
                                           (when (<= (+ a i) n)
                                             (let ((k1 (if (>= (+ a i) 0)
                                                           1
                                                           (ceiling (- a) i))))
                                               (when (<= k1 m)
                                                 (cons k1
                                                       (cons (+ a (* k1 i))
                                                             i))))))
                                         (zip-with-index as 1))))
         (xiss2 (scanl1 (lambda (acc xis)
                          (append xis
                                  (filter-map (dlambda ((x . i))
                                                (when (<= (+ x i) n)
                                                  (cons (+ x i) i)))
                                              acc)))
                        (cdr (coerce-list xiss1)))))
    (mapcar @(#'mex $(mapcar #'car))
            xiss2)))
