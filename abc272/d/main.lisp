...
(defun solve (n m)
  (let ((deltas (lcomp ((di (1+ (- n)) n)
                        (dj (1+ (- n)) n))
                    (= (+ (^2 di) (^2 dj))
                       m)
                  (cons di dj)))
        (g (make-list-array (^2 n))))
    (mapc (dlambda ((i . j))
            (setf (aref g (row-major-index i j n))
                  (->> (filter (dlambda ((di . dj))
                                 (and (<= 0 (+ i di) (1- n))
                                      (<= 0 (+ j dj) (1- n))))
                               deltas)
                       (mapcar (dlambda ((di . dj))
                                 (row-major-index (+ i di) (+ j dj) n))))))
          (collecting* ((i 0 n) (j 0 n))
            (cons i j)))
    (let ((d (bfs g 0)))
      (make-array-with-content ((i n) (j n))
        (aref d (row-major-index i j n))))))

(defun main ()
  (readlet (n m)
    (print-matrix (solve n m))))
