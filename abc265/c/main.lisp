...
(defun read-char-matrix (height width)
  (let ((mat (make-array `(,height ,width))))
    (dotimes (i height)
      (dotimes (j width)
...
(defun out-of-grid-p (i j h w)
  (or (< i 0) (>= i h)
      (< j 0) (>= j w))) 

(defun solve (h w g)
  (nlet rec ((i 0) (j 0))
    (if (char= (aref g i j) #\X)
        nil
        (multiple-value-bind (k l) (move i j (aref g i j))
          (setf (aref g i j) #\X)
          (if (out-of-grid-p k l h w)
              (list (1+ i) (1+ j))
              (rec k l))))))
                
(defun main ()
  (readlet (h w)
    (let ((g (read-char-matrix h w)))
      (aif (solve h w g)
           (join-print it)
           (println -1)))))
