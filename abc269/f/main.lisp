...
(defun nonzeros (m row c d)
  "Returns number of nonzero elements in specified area."
  (assert (<= c d))
  (let ((len (1+ (- d c))))
    (values
     (if (oddp (+ row c))
...
(defun row-sum (m row c d)
  (assert (<= c d))
  (let ((x1 (if (oddp (+ row c))
                (num m row (1+ c))
                (num m row c))))
    (sum-of-arithmetic-sequence (nonzeros m row c d)
                                2
                                x1)))

(defun sum (m a b c d)
  (let ((n-rows (1+ (- b a)))
        (row1-sum (row-sum m a c d))
        (row2-sum (row-sum m (1+ a) c d)))
    (mod+ (sum-of-arithmetic-sequence (ceiling n-rows 2)
                                      (* 2 m (nonzeros m a c d))
                                      row1-sum)
          (sum-of-arithmetic-sequence (floor n-rows 2)
                                      (* 2 m (nonzeros m (1+ a) c d))
                                      row2-sum))))

(defun solve (n m qs)
  (declare (ignore n))
  (mapcar (applier (curry #'sum m))
          qs))
