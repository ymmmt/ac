...
;; https://atcoder.jp/contests/abc269/editorial/4841
(defun ipartition (n)
  (assert (>= n 0))
  (->> (until #'zerop
              (lambda (n acc 2^k)
...
(defun convert (cs)
  (let ((counter (counter cs)))
    (mapcan (lambda (c)
              (let ((count (gethash c counter)))
                (assert count)
                (mapcar $(cons c) (ipartition count))))
            (ht-keys counter))))

(defun update (dp d count s)
  #@((simple-array fixnum) dp)
  #@(fixnum d count s)
  (minf (aref dp (+ s d))
        (+ (aref dp s) count)))
...
(defun solve (n m abs)
  (let* ((cs (mapcar (dlambda ((a . b)) (- b a)) abs))
         (fs (convert cs))
         (sa (reduce #'+ abs :key #'car))
         (dp (make-fixnum-array (1+ m) :initial-element +inf+)))
    (setf (aref dp sa) 0)
    (mapc (dlambda ((c . count))
            #@(fixnum c count d l r)
            (let* ((d (* c count))
                   (l (max 0 (- d)))
                   (r (1+ (min m (- m d)))))
              ;; c(自分自身)に関する更新と干渉しないように
              ;; 更新順を分岐する必要がある
              (cond ((plusp d) (dfor (s l r) (update dp d count s)))
                    ((minusp d) (for (s l r) (update dp d count s)))
                    (t nil))))
          fs)
    (mapcar $(ans n dp) (range (1+ m)))))
