...
(defun row-contain (s i)
  (thereis (j 0 10)
    (char= #\# (aref s i j))))

(defun col-contain (s j)
  (thereis (i 0 10)
    (char= #\# (aref s i j))))

(defun solve (s)
  (let* ((is (range 10))
         (a (find-if (curry #'row-contain s) is))
         (b (find-if (curry #'row-contain s) is :from-end t))
         (c (find-if (curry #'col-contain s) is))
         (d (find-if (curry #'col-contain s) is :from-end t)))
    `((,a ,b) (,c ,d))))
            
(defun main ()
  (let ((s (read-char-matrix 10 10)))
    (dbind (ab cd) (solve s)
      (join-print ab :key #'1+)
      (join-print cd :key #'1+))))
