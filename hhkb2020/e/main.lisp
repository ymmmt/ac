...
(defsubst blankp (s i j)
  (char= (aref s i j) #\.))

(defun cache (h w u d l r)
  (mapc-range (lambda (i)
                (mapc-range $(funcall u i) 0 w)
                (mapc-range $(funcall l i) 0 w))
              0 h)
  (mapc-drange (lambda (i)
                 (mapc-drange $(funcall d i) 0 w)
                 (mapc-drange $(funcall r i) 0 w))
               0 h))

(defun solve (h w s)
  (let* ((k (array-count-if s (eqler #\.)))
         (k->2^k (make-array-with-content ((k (1+ (* h w))))
                   (if (zerop k)
                       1
                       (mod* 2 (aref self (1- k))))))
         (2^k (aref k->2^k k)))
    (macrolet ((cells (name di dj)
                 `(if (blankp s i j)
                      (if (array-in-bounds-p s (+ i ,di) (+ j ,dj))
                          (1+ (,name (+ i ,di) (+ j ,dj)))
                          1)
                      0)))
      (with-caches ((:array (h w) :initial-element -1)
                    (:array (h w) :initial-element -1)
                    (:array (h w) :initial-element -1)
                    (:array (h w) :initial-element -1))
        (labels ((ucells (i j)
                   (cells ucells -1 0))
                 ...
                 (rcells (i j)
                   (cells rcells 0 1)))
          (cache h w #'ucells #'dcells #'lcells #'rcells)
          (labels ((f (i j)
                     (if (blankp s i j)
                         (let ((kij (+ (ucells i j) (dcells i j)
                                       (lcells i j) (rcells i j)
                                       -3)))
                           (mod- 2^k (aref k->2^k (- k kij))))
                         0)))
            (reduce #'mod+
                    (collecting* ((i 0 h) (j 0 w))
                      (cons i j))
                    :key (cons-applier #'f))))))))                
