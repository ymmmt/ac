...
(defun animal (a- a s)
  "Determines next animal from previous animal, current animal and 
current animal's string."
  (switch ((list a s) :test 'equal)
    ('(#\S #\o) a-)
    ('(#\W #\x) a-)
    (t (negate a-))))

(defun solve (s)
  (thereis ((a0 a1) (cartesian-product-elements 2 '(#\S #\W)))
    (let ((as (mvfoldl (lambda (si as a- a)
                         (let ((a+ (animal a- a si)))
                           (values (cons a+ as)
                                   a
                                   a+)))
                       (cdr s) (list a1 a0) a0 a1)))
      (when (and (eql (first as) a0)
                 (eql (second as) (animal a1 a0 (car s))))
        (nreverse (cdr as))))))

(defun main ()
  (readlet (n)
    (let ((s (coerce-list (read-line))))
      (aif (solve s)
           (join-print it :sep "")
           (println -1)))))
