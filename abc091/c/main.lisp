...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (use-package :cp/bipartite-matching :cl-user))
...
(defun connect (graph u v)
  (push v (aref graph u))
  (push u (aref graph v)))

(defun good-pair-p (x y z w)
  (and (< x z)
       (< y w)))

(defun solve (n r-edges b-edges)
  (let ((g (make-graph (2* n) '())))
    (loop for ((x . y) i) in (zip-with-index r-edges) do
      (loop for ((z . w) j) in (zip-with-index b-edges) do
        (when (good-pair-p x y z w)
          (connect g i (+ n j)))))
    (nth-value 1 (find-matching g))))

(defun main ()
  (readlet (n)
    (let ((r-edges (read-edges n))
          (b-edges (read-edges n)))
      (println (solve n r-edges b-edges)))))
