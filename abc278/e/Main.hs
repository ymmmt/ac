...
solve h w n r c xss = [[ans k l | l <- [0..w-c]] | k <- [0..h-r]]
  where ans k l  = m - count (p k l) us
        us       = sortUniq $ concat xss
        m        = length us
        p k l x  = k < minRow!x && maxRow!x <= k+r
                   && l < minCol!x && maxCol!x <= l+c
        xss'     :: [(Int, Cell)]
        xss'     = concatMap rowCells $ zip [1..] xss
        cells    :: Array Int [Cell]
        cells    = accumArray (flip (:)) [] (1, n) xss'
        minRow   = array (1, n) $ map (tupWith $ minimum . map fst . (cells!)) us
        maxRow   = array (1, n) $ map (tupWith $ maximum . map fst . (cells!)) us
        minCol   = array (1, n) $ map (tupWith $ minimum . map snd . (cells!)) us
        maxCol   = array (1, n) $ map (tupWith $ maximum . map snd . (cells!)) us
        -- minRow   = memoize $ minimum . map fst . (cells!)
        -- maxRow   = memoize $ maximum . map fst . (cells!)
        -- minCol   = memoize $ minimum . map snd . (cells!)
        -- maxCol   = memoize $ maximum . map snd . (cells!)

main :: IO ()
main = do
  [h, w, n, r, c] <- readIntList
  xss             <- readIntListsAll
  mapM_ (putStrLn . joinStr " ") $ solve h w n r c xss
