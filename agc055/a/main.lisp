...
(defun solve (n s)
  (let* ((sl (subseq s 0 n))
         ...
         (cr (counter sr))
         (perm->count
           (-> (foldl (lambda (acc perm)
                        (dbind (x y z) perm
                          (let ((min (min (gethash x cl 0)
                                          (gethash y cm 0)
                                          (gethash z cr 0))))
                            (decf (gethash x cl 0) min)
                            (decf (gethash y cm 0) min)
                            (decf (gethash z cr 0) min)
                            (if (zerop min)
                                acc
                                (acons perm min acc)))))
                      nil
                      (permutations '(#\A #\B #\C)))
               (zip-with-index 1)))
         (l-is (duplicate-item-indices sl))
         (m-is (duplicate-item-indices sm))
         (r-is (duplicate-item-indices sr)))
    (labels ((group-numberings (k is)
               (map-range (lambda (i)
                            (let ((pcs (filter (lambda (pc)
                                                 (let ((perm (car (first pc))))
                                                   (char= (nth k perm)
                                                          (char s i))))
                                               perm->count)))
                              (ecase (length pcs)
                                (1 (let ((group (second (car pcs))))
                                     group))
                                (2 (dbind (((p1 . c1) g1)
                                           ((p2 . c2) g2))
                                       pcs
                                     (if (< (aref is (mod i n)) c1)
                                         g1
                                         g2))))))
                          (* k n) (* (1+ k) n))))
      (nconc (group-numberings 0 l-is)
             (group-numberings 1 m-is)
             (group-numberings 2 r-is)))))

(defun main ()
  (readlet (n)
    (let ((s (read-line)))
      (join-print (solve n s) :sep ""))))
