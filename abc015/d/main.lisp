...
(defun solve (w n k abs)
  (with-cache (:array ((1+ w) n (1+ k)) :initial-element -1)
    (labels ((dp (width i count)
               (dbind (a . b) (svref abs i)
                 (cond ((zerop count) 0)
                       ((zerop i)
                        (if (and (= a width) (= count 1))
                            b 0))
                       ((> a width)
                        (dp width (1- i) count))
                       (t
                        (max (dp width (1- i) count)
                             (+ (dp (- width a) (1- i) (1- count))
                                b)))))))
      (maximize* ((width 1 (1+ w))
                  (count 1 (1+ k)))
        (dp width (1- n) count)))))

(defun main ()
  (readlet (w n k)
    (let ((abs (coerce-vector (read-conses n))))
      (println (solve w n k abs)))))
