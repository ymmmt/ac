...
(defun fastest-arrival-time (time ci si fi)
  (+ (if (>= time si)
         (+ si (* fi (ceiling (- time si) fi)))
         si)
     ci))

(defun solve (n csfs)
  (declare (ignore n))
  (append (maplist (lambda (sub-csfs)
                     (reduce (lambda (time csf)
                               (apply #'fastest-arrival-time
                                      time csf))
                             sub-csfs
                             :initial-value 0))
                   csfs)
...
(defun main ()
  (readlet (n)
    (let ((csfs (collect (1- n)
                  (readlet (c s f)
                    (list c s f)))))
      (printlns (solve n csfs)))))
