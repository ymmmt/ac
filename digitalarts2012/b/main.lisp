...
(defun num->char (n)
  (assert (<= 1 n 26))
  (code-char (+ n #.(char-code #\a) -1)))

(defun add-to-char (c delta)
  (num->char (+ (char->num c) delta)))

(defconstant maxlen 20)

(defun solve (cs)
  (let ((n (length cs)))
    (cond ((or (and (= n 1) (every (eqler #\a) cs))
               (and (= n maxlen) (every (eqler #\z) cs)))
           "NO")
          ((every (eqler #\z) cs)
           (concatenate 'string
                        "ay"
                        (make-string (1- n) :initial-element #\z)))
          ((= n 1)
           (coerce (list (add-to-char (char cs 0) -1)
                         #\a)
                   'string))
          (t
           (let ((sum (reduce #'+ cs :key #'char->num)))
             (mvbind (q r) (floor sum 26)
               (let ((c (if (zerop r)
                            ""
                            (make-string 1 :initial-element (num->char r))))
                     (zs (make-string q :initial-element #\z)))
                 (car (delete cs
                              (list (concatenate 'string c zs)
                                    (concatenate 'string zs c))
                              :test #'string=)))))))))
