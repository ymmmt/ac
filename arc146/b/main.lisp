...
;; 立てる。maskによって、dの最上位ビットより
;; 更に上位のaのビットは無視する(これらのビットは立てたい
;; dのビットとは無関係)。これによって
;; 最終的な値が正の数になることが保証される。
(defun cost (target a)
  (declare (optimize speed (safety 1))
           (fixnum target a))
  (let ((d (logxor target
    ...
    (if (zerop d)
        0
        (let ((mask (1- (2^ (integer-length d)))))
          (- (logand target mask)
             (logand a mask))))))

(defun solve (m k as)
  (range-foldr (lambda (i ans)
                 (let* ((ans* (+ ans (2^ i)))
                        (cost (->> (mapcar (curry #'cost ans*) as)
                                   (nbest-k k % #'<)
                                   (reduce #'+))))
                   (if (<= cost m)
                       ans*
