...
;; 操作Aの逆操作: 先頭の0をひとつ除いて残りをflipする
;; 操作Bの逆操作: 末尾の0をひとつ除く
;; この2つの逆操作の有限回の適用によってaを空列にできることは
;; 答えがYesであることの必要十分条件。

;; 次の貪欲法によって探索範囲[l,r]の長さを0にできれば
;; aを空列にできる。
;; (1) 部分列[l,r]の先頭の0を1つを取り除く。これはAの逆操作にあたる。
;; この操作に失敗してもいいのは、部分列がすべて0である場合(成功)のみで、
;; それ以外の場合に失敗すれば答えはNo。
;; (2) 部分列[l,r]の末尾の0個以上の0をすべて取り除く。これはBの逆操作にあたる。
;; (1)と(2)の適用結果で再帰する。
;; ビットのフリップを毎回するのは非効率なので
;; その代りに再帰ごとに0と1の扱いをフリップする。
(defun solve (n a)
  (labels ((flip (bit)
             (logxor bit 1))
           (rec (bit l r)
             (assert (<= l r))
             (cond ((< l r)
                    (or (loop for i from l to r
                              always (= bit (svref a i)))
                        (and (= bit (svref a l))
                             (let ((i (position (flip bit) a
                                                :start l :end (1+ r)))
                                   (j (position (flip bit) a
                                                :from-end t
                                                :start l :end (1+ r))))
                               (and (= (1+ l) i)
                                    (rec (flip bit) i j))))))
