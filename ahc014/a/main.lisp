...
(defstruct (state (:constructor state
                      (grid points ops row-edges col-edges ldiag-edges rdiag-edges score))
                  (:copier nil))
...
(defun copy-state (state)
  (with-accessors ((grid state-grid)
                   ...
                   (score state-score))
      state
    (state (copy-simple-bit-array grid)
           points
           ops
           (copy-simple-bit-array row-edges)
           (copy-simple-bit-array col-edges)
...
(defun valid-row-edge-p (grid row-edges row c1 c2)
  #@((simple-array bit) grid row-edges)
  (sortf < c1 c2)
  (always (c c1 c2)
    (and (or (= c c1)
             (blankp grid row c))
...
(defun valid-col-edge-p (grid col-edges col r1 r2)
  #@((simple-array bit) grid col-edges)
  (sortf < r1 r2)
  (always (r r1 r2)
    (and (or (= r r1)
             (blankp grid r col))
...
(defun valid-ldiag-edge-p (grid ldiag-edges r1 c1 r2 c2)
  #@((simple-array bit) grid ldiag-edges)
  (if (> r1 r2)
...
(defun valid-rdiag-edge-p (grid rdiag-edges r1 c1 r2 c2)
  #@((simple-array bit) grid rdiag-edges)
  (if (> r1 r2)
