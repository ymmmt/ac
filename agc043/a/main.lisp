...
(defun min-cost (i j prev-white-p)
  (let* ((blackp (blackp (aref *s* i j)))
         (whitep (not blackp))
         (cost (if (and prev-white-p blackp)
                   1 0)))
    (cond ((and (< i (1- *h*)) (< j (1- *w*)))
           (+ cost
               (min (min-cost (1+ i) j whitep)
                    (min-cost i (1+ j) whitep))))
          ((< i (1- *h*))
           (+ cost
               (min-cost (1+ i) j whitep)))
          ((< j (1- *w*))
           (+ cost
               (min-cost i (1+ j) whitep)))
          (t
...
(memoize 'min-cost :key #'identity :test #'equal)
