...
(defun makablep (a b c d f sugar/100g water)
  ...
  (and (<= (+ water sugar/100g)
           f)
       (natural-number-sol a b (/ water 100))
       (natural-number-sol c d sugar/100g)))

(defun solve (a b c d e f)
  (let ((wss (filter (dlambda ((s w))
                       (makablep a b c d f s w))
                     (collecting* ((w 100 (1+ f) 100)
                                   (s 0 (1+ (* (/ w 100) e))))
                       (list s w)))))
    (dbind (s w) (best (applier #'/) wss)
      (list (+ s w) s))))

(defun main ()
  (readlet (a b c d e f)
    (join-print (solve a b c d e f))))
