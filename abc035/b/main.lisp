...
(defmacro counts (items sequence &rest args)
  (let ((gseq (gensym "SEQ")))
    `(let ((,gseq ,sequence))
       (values
        ,@(mapcar (lambda (item)
                    `(count ,item ,gseq ,@args))
                  items)))))

;; 最大値は自明。
;; 最小値については、まず
;; (>= dist q)のときは、貪欲に原点から遠ざかる。
;; (< dist q)のときは、まず(- q dist)回で原点へ戻り、
;; その後原点とその隣接点を往復して止まった部分と原点との距離が答え。
(defun solve (s b)
  (multiple-value-bind (l r u d q)
      (counts (#\L #\R #\U #\D #\?) s :test #'char=)
    (let ((dist (+ (dist l r) (dist u d))))
      (ecase b
        (:max (+ dist q))
        (:min (if (>= dist q)
                  (- dist q)
