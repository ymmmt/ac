...
(defun intervals (n as)
  (let ((a->indices (list-accum-ht (alist as (range 1 (1+ n))))))
    (mapcan (curry #'map-adjacents #'cons)
            (ht-values a->indices))))

(defun r->interval (intervals)
  (ht (mapcar #'cdr intervals)
      intervals))

(defun solve (n as)
  (let ((r->interval (r->interval (intervals n as))))
    (with-memos ((dp (x)
                   (acond ((= x 1) 1)
                          ((gethash x r->interval)
                           (dbind (l . r) it
                             (if (<= (- x (dp (1- x)))
                                     l)
                                 (- r l)
