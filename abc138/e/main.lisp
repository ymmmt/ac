...
(defun solve (s u)
  (let ((n (length s))
        (s-chars (counter s))
        (u-chars (counter u)))
    (if (some (complement (mapper s-chars))
              (ht-keys u-chars))
        -1
        (let ((char-poss (accum-ht (collecting (i 0 n)
                                     (cons (char s i) i)))))
          (1+
           (foldl (lambda (i c)
                    (let* ((k (length (gethash c char-poss)))
                           (l (mod i n))
                           (pos (bisect-right (gethash c char-poss)
                                              l :end k))
                           (m (aref (gethash c char-poss)
                                    (if (= pos k)
                                        0
                                        pos))))
                      (if (<= m l) ; k=1 => m=l
                          ...
                          (+ i (- m l)))))
                  (let* ((u0 (char u 0))
                         (pos (bisect-right (gethash u0 char-poss)
                                            -1)))
                    (aref (gethash u0 char-poss) pos))
                  (slice u 1)))))))
