...
(defun solve (h w c)
  (let* ((c (make-array-with-content ((i (1+ h)) (j (1+ w)))
              (if (evenp (+ i j))
                  (aref c i j)
                  (- (aref c i j)))))
         (cumsums (make-array-with-content ((i (1+ h)) (j (1+ w)))
                    (if (or (zerop i) (zerop j))
                        ...
                        (+ (aref self i (1- j))
                           ...
                           (- (aref self (1- i) (1- j)))
                           (aref c i j))))))
    (labels ((goodp (i j k l)
               (assert (<= 1 i k h))
               (assert (<= 1 j l w))
               (zerop (- (+ (aref cumsums k l)
                            (aref cumsums (1- i) (1- j)))
                         (aref cumsums k (1- j))
                         (aref cumsums (1- i) l)))))
      (maximize* ((i 1 (1+ h))
                  (j 1 (1+ w))
...
(defun main ()
  (readlet (h w)
    (let ((c (read-matrix h w :padding t)))
      (println (solve h w c)))))
