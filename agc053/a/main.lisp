...
(defun dividablep (n s a k)
  (not (some (lambda (i)
               (mvbind (q1 r1) (floor (aref a i) k)
                 (mvbind (q2 r2) (floor (aref a (1+ i)) k)
                   (or (= q1 q2)
                       (ecase (char s i)
                         (#\< (and (= (1+ q1) q2)
                                   (> r1 r2)))
...
(defun divide (n a k)
  (with-memos ((q (j)
               ...
               (r (j)
                 (mod (aref a j) k)))
    (make-array-with-content ((i k) (j (1+ n)))
      (if (< i (r j))
          (1+ (q j))
          (q j)))))

(defun solve (n s a)
  (let ((k (binary-search 1 (+ (reduce #'max a) 2)
                          (curry #'dividablep n s a))))
    (values k (divide n a k))))
