...
(defun valid-fix-p (n fixes xs ys)
  (let ((fixed (make-fixnum-array n)))
    (mapc (dlambda ((i . x))
            (when (and (plusp (aref fixed i))
                       (/= (aref fixed i) x))
              (return-from valid-fix-p nil))
            (setf (aref fixed i) x))
          fixes)
    (and (every (lambda (fix x y)
                  (<= fix (min x y)))
                fixed xs ys)
         (coerce-list fixed))))

(defun solve (n xs ys)
  (let ((fs (nconc (fixes xs)
                   (->> (fixes (reverse ys))
                        (mapcar (dlambda ((i . y))
                                  (cons (- n i 1) y)))
                        nreverse))))
    (aif (valid-fix-p n fs xs ys)
         (let ((fixed it))
           (-> (mapcar (lambda (fix x y)
                         (if (plusp fix)
                             1
                             (min x y)))
                       fixed xs ys)
               (reduce #'mod* % :initial-value 1)))
         0)))
