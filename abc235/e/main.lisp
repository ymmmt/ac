...
(defun solve (n abs cs uvs ws)
  (let* ((edges (sort (append (pairlis abs cs)
                              (pairlis uvs ws))
                      #'< :key #'cdr))
         (q-ans (make-hashset (filter (let ((q-edges (make-hashset (pairlis uvs ws)
                                                                   :test #'equal))
                                            (ds (make-disjoint-set n)))
                                        (lambda (e)
                                          (dbind (u . v) (car e)
                                            (when (not (ds-connected-p ds u v))
                                              (or (gethash e q-edges)
                                                  (prog1 nil
                                                    (ds-unite! ds u v)))))))
                                      edges)
                              :test #'equal)))
    (mapcar (lambda (uv w)
              (if (gethash (cons uv w) q-ans)
                  "Yes"
                  "No"))
            uvs ws)))
...
(defun main ()
  (readlet (n m q)
    (mvbind (abs cs) (read-weighted-edges m)
      (mvbind (uvs ws) (read-weighted-edges q)
        (bulk-stdout
          (printlns (solve n abs cs uvs ws)))))))
