...
(defun solve (h w s)
  (let ((r (make-fixnum-array `(,h ,w) :initial-element -1))
        (c (make-fixnum-array `(,h ,w) :initial-element -1))
        (d (make-fixnum-array `(,h ,w) :initial-element -1))
        (p (make-fixnum-array `(,h ,w) :initial-element -1)))
    (labels ((wallp (i j)
               (char= (aref s i j) #\#))
             (dp-cumsum-row (i j)
               (cond ((or (= j w) (wallp i j))
                      0)
                     ((/= (aref r i j) -1)
                      (aref r i j))
                     (t
                      (setf (aref r i j)
                            (md (+ (dp-cumsum-row i (1+ j))
                                   (dp i j)))))))
             (dp-cumsum-col (i j)
               (cond ((or (= i h) (wallp i j))
                      0)
                     ((/= (aref c i j) -1)
                      (aref c i j))
                     (t
                      (setf (aref c i j)
                            (md (+ (dp-cumsum-col (1+ i) j)
                                   (dp i j)))))))
             (dp-cumsum-diag (i j)
               (cond ((or (= i h) (= j w)
                          (wallp i j))
