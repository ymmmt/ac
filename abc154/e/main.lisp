...
(defun solve (n k)
  (if (zerop k)
      ...
      (let* ((ds (num->digits n))
             (h (car ds))
             (n-digits (n-digits n 10)))
        (+
         ;; (n-digits未満)桁で条件を満たすものの数
         (sum (d 1 n-digits)
           (* (expt 9 k) (choose (1- d) (1- k))))
         ;; n-digits桁で、最上位桁がnと同じものの数
         (solve (digits->num (cdr ds)) (1- k))
         ;; n-digits桁で、最上位桁がn未満のものの数
         (* (1- h)
            (expt 9 (1- k))
            (choose (1- n-digits) (1- k)))))))
