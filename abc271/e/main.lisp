...
;; (defun solve (n k abc e)
;;   (let* ((edge (map 'vector @((arefer abc) #'1-) e))
;;          (i->movable-js (let ((a->js (make-hash-table)))
;;                           (make-array-with-content ((i k :downward t))
;;                             (dbind (a b c) (aref edge i)
;;                               (prog1 (gethash b a->js)
;;                                 (push i (gethash a a->js))))))))
;;     (with-memos ((dp (i)
;;                    (dbind (a b c) (aref edge i)
;;                      (if (= b n)
;;                          c
;;                          (let ((js (aref i->movable-js i)))
;;                            (if js
;;                                (+ c (minimize (j js) (dp j)))
;;                                +inf+))))))
;;       (let ((is (position-if-all @((eqler 1) #'first) edge)))
;;         (if is
;;             (minimize (i is) (dp i))
;;             +inf+)))))
...
;; https://atcoder.jp/contests/abc271/editorial/4924
(defun solve (n abc e)
  (let ((edge (map 'vector @((arefer abc) #'1-) e))
        (dp (make-array-with-content ((i (1+ n)))
              (if (= i 1) 0 +inf+))))
    (map nil (dlambda ((a b c))
               (minf (aref dp b)
                     (+ (aref dp a) c)))
         edge)
    (aref dp n)))
...
(defun main ()
  (readlet (n m k)
    (declare (ignore k))
    (let ((abc (coerce-vector (read-lists m)))
          (e (readvector)))
      (println (let ((ans (solve n abc e)))
                 (if (< ans +inf+)
                     ans
                     -1))))))
