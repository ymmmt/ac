...
(defun solve (n s abs)
  (let ((a (-> (mapcar #'car abs) coerce-vector pad))
        (b (-> (mapcar #'cdr abs) coerce-vector pad)))
    (with-memos ((dp (sum i)
                   (cond ((minusp sum) nil)
                         ((zerop i) (zerop sum))
                         (t
      ...
      (if (dp s n)
          (values "Yes"
                  (nlet rec ((i n) (acc nil) (sum s))
                    (if (zerop i)
                        (coerce acc 'string)
                        (if (dp (- sum (aref a i)) (1- i))
                            (rec (1- i) (cons #\H acc) (- sum (aref a i)))
                            (rec (1- i) (cons #\T acc) (- sum (aref b i)))))))
          "No"))))

(defun main ()
  (readlet (n s)
    (let ((abs (read-conses n)))
      (mvbind (y-or-n ans) (solve n s abs)
        (println y-or-n)
        (when ans (println ans))))))
