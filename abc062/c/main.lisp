...
;; 2本の切断のうち1本は確定で、その面積はTHE-OTHER-AREA
;; 残った領域を2等分して答えを求める
;; 残った領域はできるかぎり均等に分割したい
(defun div2 (h w the-other-area)
  (let ((area (* h w)))
    (multiple-value-bind (b c)
        (if (or (evenp h) (evenp w))
            (values (/ area 2) (/ area 2))
            (let ((d (min h w)))
              (values (/ (+ area d) 2)
                      (/ (- area d) 2))))
      (- (max the-other-area b c)
         (min the-other-area b c)))))
               
(defun solve (h w)
  (min (minimize (left 1 w)
         (div2 h (- w left) (* h left)))
       (minimize (up 1 h)
         (div2 (- h up) w (* up w)))
       ;; 縦2本または横2本の切断の最良値
       (if (or (zerop (mod h 3))
