...
(defun xl->interval (xl)
  (destructuring-bind (x . l) xl
    (cons (- x l) (+ x l))))

(defun solve (n xls)
  (declare (ignore n))
  (let ((intervals (sort (mapcar #'xl->interval xls) #'<
                         :key #'cdr)))
    (nlet rec ((is (cdr intervals))
               (last-r (cdr (car intervals)))
               (acc 1))
      (if (null is)
          acc
          (destructuring-bind (l . _) (car is)
            (if (<= last-r l)
                (rec (cdr is) (cdr (car is)) (1+ acc))
                (rec (cdr is) last-r acc)))))))
