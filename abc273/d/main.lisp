...
(defun radd (walls r c)
  (unless (gethash r walls)
    (setf (gethash r walls)
          (make-adj-array)))
  (vector-push-extend c (gethash r walls))
  walls)

(defun cadd (walls r c)
  (unless (gethash c walls)
    (setf (gethash c walls)
          (make-adj-array)))
  (vector-push-extend r (gethash c walls))
  walls)

(defmacro %move (ht-key skey walls search delta maxmin default)
  (sb-ext::once-only ((default default))
    `(or (aand (gethash ,ht-key ,walls)
               (,search it ,skey)
               (,maxmin (,delta it) ,default))
         ,default)))
                 
(defun move (h w rwalls cwalls r c d l)
  (ecase d
    (L (list r
             (%move r c rwalls previous-element 1+ max
                    (max 1 (- c l)))))
    (R (list r
             (%move r c rwalls next-element 1- min
                    (min w (+ c l)))))
    (U (list (%move c r cwalls previous-element 1+ max
                    (max 1 (- r l)))
             c))
    (D (list (%move c r cwalls next-element 1- min
                    (min h (+ r l)))
             c))))

(defun solve (h w rs cs n rcs dls)
  (labels ((walls (add-fn)
             (foldl (dlambda (walls (r . c))
                      (funcall add-fn walls r c))
                    (make-hash-table :size n)
                    rcs)))
    (let ((rws (walls #'radd))
          (cws (walls #'cadd)))
      (mapc (lambda (r) (sortf (gethash r rws) #'<))
            (ht-keys rws))
      (mapc (lambda (c) (sortf (gethash c cws) #'<))
            (ht-keys cws))
      (cdr (scanl (dlambda ((r c) (d . l))
                    (move h w rws cws r c d l))
                  (list rs cs)
                  dls)))))

(defun main ()
  (readlet (h w rs cs n)
    (let ((rcs (read-conses n)))
      (readlet (q)
        (let ((dls (collect q
                     (cons (read) (read-fixnum)))))
          (bulk-stdout
            (mapc #'join-print
                  (solve h w rs cs n rcs dls))))))))
