...
(defun solve (xcs)
  (let* ((cxs (mapcar (dlambda ((x . c)) (cons c x)) xcs))
         (c->left (accum-ht cxs +inf+ #'min))
         (c->right (accum-ht cxs +ninf+ #'max))
         (cs (sort (ht-keys c->left) #'<)))
    (mvbind (l-cost r-cost xl xr)
        (mvfoldl (lambda (c l-cost r-cost xl xr)
                   (let* ((l (gethash c c->left))
                          (r (gethash c c->right))
                          (w (dist l r)))
                     (values (+ w (min (+ l-cost (dist r xl))
                                       (+ r-cost (dist r xr))))
                             (+ w (min (+ l-cost (dist l xl))
                                       (+ r-cost (dist l xr))))
                             l r)))
                 cs 0 0 0 0)
      (min (+ l-cost (dist xl 0))
           (+ r-cost (dist xr 0))))))                       

(defun main ()
  (readlet (n)
    (let ((xcs (read-conses n)))
      (println (solve xcs)))))
