...
;; ほぼ解説の通りだが、TLEを回避するために
;; x(n, d)はmod mで計算してしまうことで効率化する。
(defun solve (n m)
  (with-cache (:array ((1+ n) 10) :initial-element 0)
    (labels ((x (len d)
               (assert (>= len 1))
               (assert (<= 0 d 9))
               (if (= len 1)
      ...
      (labels ((goodp (len d)
        ...
        (let* ((ls (range 1 (1+ n)))
               (cands (lcomp ((d 1 10))
                          (let ((n0 (find-if (rcurry #'goodp d) ls)))
                            (when n0
                              (cons (* (floor n n0) n0)
                                    d)))
                        it)))
          (when cands
            (dbind (len . d) (best cands :order (cons-order #'< #'<))
              (repeat len d))))))))
