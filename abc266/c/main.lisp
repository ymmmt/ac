...
(defun sign (px py qx qy rx ry)
  (- (* (- px rx)
        (- qy ry))
     (* (- qx rx)
        (- py ry))))

(defun outside-of-triangle-p (tri s)
  (dbind (p1 p2 p3) tri
    (dbind (sx sy) s
      (let ((signs (mapcar (dlambda (((ax ay) (bx by)))
                             (sign sx sy ax ay bx by))
                           (list (list p1 p2)
                                 (list p2 p3)
                                 (list p3 p1)))))
        (and (some #'plusp signs)
             (some #'minusp signs))))))

(defun solve (points)
  (every (lambda (p)
           (outside-of-triangle-p (remove p points :test #'equal)
                                  p))
         points))

(defun main ()
  (let ((points (collect 4
                  (readlet (x y)
                    (list x y)))))
    (println (if (solve points)
                 "Yes"
                 "No"))))
