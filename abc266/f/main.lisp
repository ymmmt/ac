...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (use-package :cp/find-cycle :cl-user))
...
(defun solve (n uvs qs)
  (let* ((g (make-graph n uvs))
         (in-cycle (find-cycle g :undirected t))
         (c (length in-cycle))
         (vertex->tree-id (make-array n :initial-element -1)))
    (labels ((dfs (vs id)
               (when vs
                 (let ((u (car vs)))
                   (setf (aref vertex->tree-id u) id)
                   (dfs (append (filter (compose (eqler -1)
                                                 (arefer vertex->tree-id))
                                        (aref g u))
                                (cdr vs))
                        id)))))
      (mapc (lambda (v id)
              (setf (aref vertex->tree-id v) id))
            in-cycle (range c))
      (mapc (lambda (v id)
              (dfs (list v) id))
            in-cycle (range c))
      (mapcar (dlambda ((x . y))
                (if (= (aref vertex->tree-id x)
                       (aref vertex->tree-id y))
                    "Yes"
                    "No"))
              qs))))
...
(defun main ()
  (readlet (n)
    (let ((uvs (read-edges n)))
      (readlet (q)
        (let ((qs (collect q
                    (readlet (x y) (cons (1- x) (1- y))))))
          (bulk-stdout
            (printlns (solve n uvs qs))))))))
