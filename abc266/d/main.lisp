...
(defconstant time-max (expt 10 5))

(defun solve (snks)
  (let ((snk (make-hash-table :test 'equal)))
    (mapc (dlambda ((time x a))
            (setf (gethash (cons time x) snk)
                  a))
          snks)
    (with-memos ((dp (time x)
                   (assert (<= 0 x 4))
                   (cond ((and (zerop time) (zerop x))
                          0)
                         ((and (zerop time) (plusp x))
                          +ninf+)
                         (t
                          (+ (gethash (cons time x) snk 0)
                             (max (if (> x 0)
                                      (dp (1- time) (1- x))
                                      +ninf+)
                                  (dp (1- time) x)
