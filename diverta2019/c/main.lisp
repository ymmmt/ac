...
(defun count-ABs (s)
  (count-if (lambda (i) (string= "AB" (subseq s i (+ i 2))))
            (range (1- (length s)))))

(defun ends-with-A (s)
  (char= #\A (char s (1- (length s)))))

(defun starts-with-B (s)
  (char= #\B (char s 0)))

(defun solve (n ss)
  (declare (ignore n))
  (let ((a (count-if (conjoin #'ends-with-A (complement #'starts-with-B))
                     ss))
        (b (count-if (conjoin #'starts-with-B (complement #'ends-with-A))
                     ss))
        (c (count-if (conjoin #'ends-with-A #'starts-with-B)
                     ss))
        (d (reduce #'+ ss :key #'count-ABs)))
    ;; B***Aという文字列をc個つなげることでABが(1- c)個できる。
    (+ (max 0 (1- c))
        (min a b)
        (if (plusp c)
            (cond ((= a b 0) 0)
