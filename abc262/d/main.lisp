...
(defun solve (n a)
  (let ((a (pad a))
        (ans 0))
    (loop for mod from 1 to n do
      (let ((dp (make-array `(,(1+ n) ,(1+ n), n) :element-type 'fixnum)))
        (setf (aref dp 0 0 0) 1)
        (loop for i from 1 to n
              for ai = (aref a i) do
                (setf (aref dp i 0 0) 1)
                (loop for size from 1 to i do
                  (loop for mod-sum below mod do
                    (setf (aref dp i size mod-sum)
                          (md (+ (aref dp (1- i) size mod-sum)
                                 (aref dp (1- i) (1- size) (mod (- mod-sum ai) mod))))))))
        (key-incf ans
                  (aref dp n mod 0)
                  md)))
    ans))
