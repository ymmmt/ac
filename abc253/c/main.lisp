...
(defun ans (q acc ms)
  (ecase (first q)
    (1 (let ((x (second q)))
         (values acc (mset-insert ms x))))
    (2 (dbind (x c) (cdr q)
         (let ((count (min c (mset-count ms x))))
           (values acc
                   (if (plusp count)
                       (mset-delete ms x :count count)
                       ms)))))
    (3 (values (cons (- (mset-last ms) (mset-first ms))
                     acc)
               ms))))

(defun solve (qs)
  (nreverse (mvfoldl #'ans qs nil nil)))
