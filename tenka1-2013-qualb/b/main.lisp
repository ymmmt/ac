...
(defun solve (l qs)
  (labels ((ret (msg acc)
             (return-from solve
               (nreverse (cons msg acc)))))
    (->> (mvfoldl (lambda (q acc stack size)
                    (ecase (car q)
                      (push (dbind (n m) (cdr q)
                              (cond ((> (+ size n) l)
                                     (ret 'full acc))
                                    ((or (null stack)
                                         (/= m (caar stack)))
                      ...
                      (pop (dbind (n) (cdr q)
                             (if (> n size)
                                 (ret 'empty acc)
                                 (values acc
                                         (nlet rec ((s stack) (n n))
                                           (if (zerop n)
                                               s
                                               (dbind (m . count) (car s)
                                                 (if (>= n count)
                                                     (rec (cdr s) (- n count))
                                                     (acons m (- count n) (cdr s))))))
                                         (- size n)))))
                      (top (if (null stack)
                               (ret 'empty acc)
                               (values (cons (caar stack) acc)
                                       stack
                                       size)))
                      (size (values (cons size acc)
                                    stack
                                    size))))
                  qs nil nil 0)
         (ret 'safe))))
...
(defun main ()
  (readlet (q l)
    (let ((qs (collect q
                (ecase (read)
                  (push (list 'push (read-fixnum) (read-fixnum)))
                  (pop (list 'pop (read-fixnum)))
                  (top (list 'top))
                  (size (list 'size))))))
      (bulk-stdout
        (printlns (solve l qs))))))
