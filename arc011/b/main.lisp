...
(defun string->number (dict s)
  (coerce (filter-map (mapper dict)
...
(defun solve (n ws)
  ...
  (let ((dict (make-hash-table))
    ...
    (dolist (cs '((#\B #\C)
      ...
      (dolist (c cs)
        (setf (gethash c dict)
              (digit-char (mod num 10)))))
    (remove "" (mapcar (curry #'string->number dict) ws)
            :test #'string=)))

(defun main ()
  (readlet (n)
    (let ((ws (split-string (read-line) :delimiterp (eqler #\Space))))
      (aif (solve n ws)
           (join-print it)
           (terpri)))))
