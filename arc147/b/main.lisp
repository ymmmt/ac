...
(defun swap (ps p->pos i j)
  (let ((p (aref ps i))
        (q (aref ps j)))
    (rotatef (aref ps i) (aref ps j))
    (rotatef (aref p->pos p) (aref p->pos q))))
...
(defun bmoves (ps p->pos from to)
  (assert (>= from to))
  (nlet rec ((pos from) (ops nil))
    (if (= pos to)
        (nreverse ops)
        (progn
          (swap ps p->pos pos (- pos 2))
          (rec (- pos 2)
               (cons (op-b (- pos 2)) ops))))))

;; https://atcoder.jp/contests/arc147/editorial/4668
(defun solve (n ps)
  (let ((ps (coerce-vector (cons 0 ps)))
        (p->pos (arr ps (range 1 (1+ n)) (1+ n))))
    (labels ((good-index-p (p)
               (oddity= p (aref p->pos p))))
      (mvbind (b-ops odd-pos)
          (nlet rec ((p 1) (even-pos 2) (odd-pos 1) (ops nil))
            (if (> p n)
                (values (nreverse ops) odd-pos)
                (if (good-index-p p)
                    (rec (1+ p) even-pos odd-pos ops)
                    (if (evenp (aref p->pos p))
                        (rec (1+ p)
                             (+ even-pos 2)
                             odd-pos
                             (nreconc (bmoves ps p->pos (aref p->pos p) even-pos)
                                      ops))
                        (rec (1+ p)
                             even-pos
                             (+ odd-pos 2)
                             (nreconc (bmoves ps p->pos (aref p->pos p) odd-pos)
                                      ops))))))
        (let ((a-ops (map-range (lambda (i)
                                  (swap ps p->pos i (1+ i))
                                  (op-a i))
                                1 odd-pos 2))
              (sort-ops (nlet rec ((p 1) (ops nil))
                          (if (= p n)
                              (nreverse ops)
                              (if (= p (aref p->pos p))
                                  (rec (1+ p) ops)
                                  (rec (1+ p)
                                       (nreconc (bmoves ps p->pos (aref p->pos p) p)
                                                ops)))))))
          (nconc b-ops
                 a-ops
                 sort-ops))))))
