...
(defconstant +factor+ (expt 10 9))

(defun interval (a xi xj vi vj)
  #@(fixnum +factor+ a xi xj vi vj)
  (assert (/= vi vj))
  (let ((p (floor (* +factor+ (- xj xi))
                  (- vi vj)))
        (q (floor (* +factor+ (+ a (- xj) xi))
                  (- vj vi))))
    (if (<= p q)
        (values p (1+ q))
        (values q (1+ p)))))

;; https://atcoder.jp/contests/abc274/editorial/5021
;; https://atcoder.jp/contests/abc274/submissions/35914105
             ...
             (dbind (wi xi vi) (aref wxvs i)
               (let* ((w (reduce #'+ wxvs
                 ...
                 (map nil (dlambda ((wj xj vj))
                            (mvbind (l r) (interval a xi xj vi vj)
                              (if (< l 0)
                                  (incf (gethash -1 time->w-delta 0) wj)
                                  (incf (gethash l time->w-delta 0) wj))
                              (if (<= r 0)
                                  (decf (gethash -1 time->w-delta 0) wj)
...
(defun main ()
  (readlet (n a)
    (let ((wxvs (coerce-vector (read-lists n))))
      (println (solve n a wxvs)))))
