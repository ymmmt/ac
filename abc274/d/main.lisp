...
(defun existp (xs value)
  (let* ((x (pad (coerce-vector xs)))
         (n (length xs))
         (dp (make-bit-array `(,(1+ n) 20001)))
         (value (+ value 10000)))
    (setf (aref dp 0 10000) 1)
    (dotimes (i (1+ n))
      (dotimes (v 20001)
        (let ((v1 (- v (aref x i)))
              (v2 (+ v (aref x i))))
          (when (or (and (array-in-bounds-p dp (1- i) v1)
                         (plusp (aref dp (1- i) v1)))
                    (and (array-in-bounds-p dp (1- i) v2)
                         (plusp (aref dp (1- i) v2))))
            (setf (aref dp i v) 1)))))
    (plusp (aref dp n value))))
                     
(defun solve (n x y as)
  (and (existp (cdr (position-filter #'evenp as))
               (- x (car as)))
       (existp (position-filter #'oddp as)
               y)))
