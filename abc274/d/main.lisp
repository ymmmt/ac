...
(defun realizable-subsum-p (list subsum)
  (assert (every #'plusp list))
  (let* ((sum (reduce #'+ list))
         (x (pad (coerce-vector list)))
         (n (1- (length x)))
         (dp (make-bit-array `(,(1+ n) ,(1+ sum)))))
    (when (<= 0 subsum sum)
      (setf (aref dp 0 0) 1)
      (dotimes (i (1+ n))
        (dotimes (s (1+ sum))
          (let ((s* (- s (aref x i))))
            (when (or (and (array-in-bounds-p dp (1- i) s)
                           (plusp (aref dp (1- i) s)))
                      (and (array-in-bounds-p dp (1- i) s*)
                           (plusp (aref dp (1- i) s*))))
              (setf (aref dp i s) 1)))))
      (plusp (aref dp n subsum)))))
                     
(defun solve (n x y as)
  (flet ((realizable-subsum-p (as subsum)
           (realizable-subsum-p (mapcar #'2* as)
                                (+ subsum (reduce #'+ as)))))
    (and (realizable-subsum-p (cdr (position-filter #'evenp as))
                              (- x (car as)))
         (realizable-subsum-p (position-filter #'oddp as)
                              y))))

(defun main ()
  (readlet (n x y)
    (let ((as (readlist)))
      (println (if (solve n x y as)
                   "Yes"
                   "No")))))
