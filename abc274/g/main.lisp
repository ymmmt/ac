...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (use-package :cp/max-flow :cl-user))

(eval-when (:compile-toplevel :load-toplevel :execute)
  (use-package :cp/dinic :cl-user))
...
(defmacro %camera-pos (name di dj)
  `(when (and (array-in-bounds-p s i j)
              (blankp s i j))
     (or (,name (+ i ,di) (+ j ,dj))
         (row-major-index i j w))))

(defun solve (h w s)
  (let* ((hw (* h w))
         (2hw (* 2 h w))
         (g (make-list-array (+ 2hw 2))))
    (with-memos ((f (i j)
                   (%camera-pos f -1 0))
                 (g (i j)
                   (%camera-pos g 0 -1)))
      (mapc (curry* #'add-edge g 2hw % 1) (range hw))
      (mapc (curry* #'add-edge g % (1+ 2hw) 1) (range hw 2hw))
      (dotimes (i h)
        (dotimes (j w)
          (let ((fij (f i j))
                (gij (g i j)))
            (when (and fij gij)
              (add-edge g fij (+ gij hw) +inf+)))))
      (max-flow! g 2hw (1+ 2hw)))))
