...
(defmacro iterate (n successor initial-value &rest initial-args)
  (let ((g (gensym "ARG"))
        (gs (loop repeat (length initial-args)
                  collect (gensym "ARG"))))
    (sb-ext::once-only ((successor successor))
      `(let ((,g ,initial-value)
             ,@(mapcar #'list gs initial-args))
         (loop repeat ,n
               collect ,g
               do (setf (values ,@(cons g gs))
                        (apply ,successor ,g ,gs)))))))
...
(defvar *fermat-2-powers*
  #(2 4 16 256 65536 4294967296 18446744073709551616))

(defsubst fermat-2-power-less-than-or-equal-to (x)
  (assert (>= x 2))
  (let ((n (1- (position-if #>x *fermat-2-powers*))))
    (values (aref *fermat-2-powers* n)
            n)))
...
(defsubst nim* (&rest integers)
  (foldl #'nim*2 1 integers))

;; ;; https://math.stackexchange.com/questions/909304/algorithm-to-multiply-nimbers
;; ;; https://judge.yosupo.jp/problem/nim_product_64
;; (defmemo (nim*2 :key #'identity :test #'equal) (a b)
;;   (cond ((or (< a 2) (< b 2))
;;          (* a b))
;;         ((> a b)
;;          (nim*2 b a))
;;         (t                          ; (and (<= a b) (>= a 2) (>= b 2))
;;          (let ((fa (fermat-2-power-less-than-or-equal-to a))
;;                (fb (fermat-2-power-less-than-or-equal-to b)))
;;            (mvbind (a1 a2) (floor a fa)
;;              (mvbind (b1 b2) (floor b fb)
;;                (let ((a-fermat-p (and (= a1 1) (= a2 0)))
;;                      (b-fermat-p (and (= b1 1) (= b2 0))))
;;                  (cond ((and a-fermat-p b-fermat-p (= a b))
;;                         (* 3/2 a))
;;                        ((and a-fermat-p b-fermat-p (< a b))
;;                         (* a b))
;;                        ((and b-fermat-p (< a b))
;;                         (* a b))
;;                        ((< fa fb)
...
;;                               (nim*2 a b2)))
;;                        (t               ; (= fa fb)
;;                         (nim+ (nim* a1 b1 (* 3/2 fb))
;;                               (nim*2 (nim+ (nim*2 a1 b2)
;;                                            (nim*2 a2 b1))
;;                                      fb)
;;                               (nim*2 a2 b2)))))))))))

(defun %nim*2 (a b rec-fn)
  (mvbind (f d) (fermat-2-power-less-than-or-equal-to (max a b))
    (let* ((p (2^ d))
           (mask (1- f))
           (a1 (ash a (- p)))
           (a2 (logand a mask))
           (b1 (ash b (- p)))
           (b2 (logand b mask))
           (a1b1 (funcall rec-fn a1 b1)))
      (nim+ (* (nim+ a1b1
                     (funcall rec-fn a1 b2)
                     (funcall rec-fn a2 b1))
               f)
            (funcall rec-fn a1b1 (/ f 2))
            (funcall rec-fn a2 b2)))))
...
(defun longest-common-prefix (rolling-hash1 rolling-hash2 left1 right1 left2 right2)
  (let* ((n1 (1+ (- right1 left1)))
         (n2 (1+ (- right2 left2)))
         (n (min n1 n2)))
    (assert (plusp n))
    (labels ((common-p (length)
               (= (funcall rolling-hash1 left1 (+ left1 length -1))
                    ...
                    (let* ((n1 (1+ (- b a)))
                           ...
                           (lcp-len
                             (longest-common-prefix
                              (lambda (l r)
                                (nim+ (rhash-sub (+ l a) (+ r a))
                                      (rhash-sub (+ l c) (+ r c))))
                              #'rhash-sub
                              0 (- b a) e f)))
                      (cond ((and (= lcp-len n) (= n1 n2)) nil)
                            ((= lcp-len n)
                             (< n1 n2))
                            (t          ; (< lcp-len n)
                             (< (nim+ (aref as (+ a lcp-len))
                                      (aref as (+ c lcp-len)))
                                (aref as (+ e lcp-len)))))))
                  qs))))))
...
(defun main ()
  (readlet (n q)
    (let ((as (readvector))
          (qs (mapcar $(mapcar #'1-) (read-lists q))))
      (bulk-stdout
        (mapc (lambda (ans)
                (println (if ans "Yes" "No")))
              (solve n as qs))))))
