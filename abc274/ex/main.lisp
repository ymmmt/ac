...
(defun most-significant-bit (integer)
  (assert (plusp integer))
  (1- (integer-length integer)))

;; matrix == simple-array of column vector
;; Interpret uint64 as column vector
(deftype vec () 'uint64)
(deftype mat () '(simple-array vec (64)))

;; https://atcoder.jp/contests/abc274/editorial/5095
;; https://atcoder.jp/contests/abc274/submissions/35934684
(defun random-matrix ()
  (coerce-vector (collect 64 (random #.(ash 1 64)))))

;; (defun unit-matrix ()
;;   (make-array-with-content ((i 64))
;;     (if (zerop i)
;;         (ash 1 63)
;;         (ash (aref self (1- i)) -1))))
...
(defun mat-mat* (m1 m2)
  (make-array-with-content ((j 64))
    (mat-vec* m1 (aref m2 j))))

(defun longest-common-prefix (dp a b c d e f)
  (let* ((n1 (1+ (- b a)))
         (n2 (1+ (- f e)))
         (n (min n1 n2)))
    (assert (plusp n))
    (labels ((commonp (k offset)
               (= (logxor (funcall dp k (+ a offset))
                          (funcall dp k (+ c offset)))
                  (funcall dp k (+ e offset))))
             (rec (k offset)
               (cond ((= offset n) 0)
                     ((zerop k)
                      (if (commonp offset k)
                     ...
                     (t
                      (rec (1- k) offset)))))
      (rec 0 (most-significant-bit n)))))                                     

(with-cache (:array (20) :element-type '(or null mat) :initial-element nil)
  (defun M (k)
    (if (zerop k)
        (random-matrix)
        (let ((m (M (1- k))))
...
;;   (if (zerop i)
;;       (unit-matrix)
;;       (let ((k (most-significant-bit i)))
;;         (mat-mat* (M k) (F (mod i (2^ k)))))))

(defun solve (n as qs)
  (with-cache (:array (20 n) :element-type '(integer -1 #.(expt 2 64)) :initial-element -1)
    (labels ((dp (k i)
               ;; iから始まる長さ2^kの部分列のhash
               (if (zerop k)
                   (aref as i)
                   (logxor (dp (1- k) i)
                           (mat-vec* (M (1- k))
      ...
      (mapcar (dlambda ((a b c d e f))
                (let* ((lcp-len (longest-common-prefix #'dp a b c d e f))
                       (n1 (1+ (- b a)))
                       (n2 (1+ (- f e)))
                       (n (min n1 n2)))
