...
(defun solve (n m xys pqs)
  (let ((2^n (2^ n))
        (2^m (2^ m))
        (xys (concatenate 'vector xys pqs)))
    (with-cache (:array ((+ n m) (2^ (+ n m))) :initial-element -1)
      (labels ((dp (i state)
                 (dbind (x . y) (aref xys i)
                   (let* ((boosters (boosters state))
                          (v (2^ (logcount (boosters state)))))
                     (if (= (1+ (visited state)) 2^n)
                         (min (minimize (j 0 m)
                                (visit v x y j state))
                              (time-needed v x y 0 0))
                         (min (minimize (i 0 n)
                                (visit v x y (+ i m) state))
                              (minimize (j 0 m)
                                (visit v x y j state)))))))
               (visit (v x y i state)
                 (if (logbitp i state)
                     +inf+
                     (dbind (xi . yi) (aref xys i)
                       (+ (time-needed v x y xi yi)
                          (dp i (bs-add i state))))))
               (visited (state)
                 (ash state (- m)))
               (boosters (state)
