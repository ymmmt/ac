...
(defun priority> (xi xj)
  (destructuring-bind (i . si) xi
    (destructuring-bind (j . sj) xj
      (or (char< si sj)
          (and (char= si sj)
               (> i j))))))

(defun solve (n s)
  (let ((chars (sort (map 'list (let ((i -1))
                                  (lambda (c)
                                    (cons (incf i) c)))
                          s)
                     #'priority>)))
    (labels ((rec (chars start end acc)
               (if (or (null chars) (< (- end start) 2))
                   (nreverse acc)
                   (let ((c (char s start)))
                     (destructuring-bind (i . si) (car chars)
                       (cond ((char<= c si)
                              (rec chars (1+ start) end acc))
                             ((and (< start i end)
                                   (char> c si))
                              (rec (cdr chars)
                                   (1+ start) i
                             ...
                             (t
                              (rec (cdr chars)
                                   start end
                                   acc))))))))
      (loop for (i . j) in (rec chars 0 n nil) do
        (rotatef (char s i) (char s j)))
      s)))
