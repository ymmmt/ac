...
(defun solve (n s)
  (declare (ignore n))
  (let ((s (map 'list #'digit-char-p s))
        (seen1 (make-bit-array 10))
        (seen2 (make-bit-array '(10 10)))
        (seen3 (make-bit-array '(10 10 10))))
    (dolist (si s)
      (dotimes (d1 10)
        (dotimes (d2 10)
          (when (plusp (aref seen2 d1 d2))
            (setf (aref seen3 d1 d2 si) 1))))
      (dotimes (d1 10)
        (when (plusp (aref seen1 d1))
          (setf (aref seen2 d1 si) 1)))
      (setf (aref seen1 si) 1))
    (array-sum seen3)))
