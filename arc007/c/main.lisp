...
(defun compose-pat (pat c offset)
  (let ((n (length pat))
        (res (copy-seq pat)))
    (mapc (lambda (i)
            (let ((j (mod (+ i offset) n)))
              (when (char= (char c i) #\o)
                (setf (char res j) #\o))))
          (range 0 n))
    res))
...
(defun solve (c)
  (let ((n (length c)))
    (labels ((rec (pat seen)
               (if (every (eqler #\o) pat)
                   0
                   (1+ (minimize (offset 0 n)
                         (if (member offset seen)
                             +inf+
                             (rec (compose-pat pat c offset)
                                  (cons offset seen))))))))
      (1+ (rec c '(0))))))
