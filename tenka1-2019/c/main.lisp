...
(defun solve (n s)
  (if (or (every #'whitep s) (every #'blackp s))
      0
      (label-subarray-sum (w (map 'vector (quantizer #'whitep) s))
        (label-subarray-sum (b (map 'vector (quantizer #'blackp) s))
          (minimize (i -1 n)
            (+ (b-subarray-sum 0 i)
               (w-subarray-sum (1+ i) (1- n))))))))
