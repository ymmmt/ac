...
;; https://atcoder.jp/contests/arc150/editorial/4866
(defun answer (n k s)
  (let* ((s (-> (string-right-trim '(#\Return) s)
                coerce-list))
         (m (count #\1 s)))
    (label-subarray-sum (0s (coerce-vector (mapcar (quantizer (eqler #\0)) s)))
      (label-subarray-sum (1s (coerce-vector (mapcar (quantizer (eqler #\1)) s)))
        (labels ((goodp (i)
                   (let ((j (+ i k -1)))
                     (and (= (0s-subarray-sum i j) 0)
                          (= (1s-subarray-sum i j) m)))))
          (if (= (count-if #'goodp (range 0 (+ n (- k) 1)))
                 1)
              "Yes"
              "No"))))))
...
(defun main ()
  (readlet (q)
    (let ((qs (collect q
                (readlet (n k)
                  (unless (find (peek-char) "01?")
                    (read-line))
                  (list n k (string-right-trim '(#\Return) (read-line)))))))
      (bulk-stdout
        (printlns (solve qs))))))
