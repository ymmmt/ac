...
(defun solve (n rhs)
  (let ((rs (coerce-vector (sort (mapcar #'car rhs) #'<)))
        (counts (counter rhs :test 'equal)))
    (mapcar (dlambda ((r . h))
              (let* ((win (+ (bisect-left rs r)
                             (gethash (cons r (mod (1+ h) 3)) counts 0)))
                     (draw (1- (gethash (cons r h) counts)))
                     (lose (- (1- n) win draw)))
                (list win lose draw)))
            rhs)))
