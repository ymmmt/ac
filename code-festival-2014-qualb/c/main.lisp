...
(defun solve (s u v)
  (let* ((n (/ (length s) 2))
         ...
         (cu (counter u))
         (cv (counter v))
         (s* (set-difference (ht-keys cv) (ht-keys cu)))
         (u* (set-difference (ht-keys cv) (ht-keys cs)))
         (su* (intersections (ht-keys cs)
                             (ht-keys cu)
                             (ht-keys cv))))
    (and (every (lambda (c)
                  (>= (+ (gethash c cs 0) (gethash c cu 0))
                      (gethash c cv 0)))
                (ht-keys cv))
         (>= (+ (reduce #'+ s* :key (mapper cv))
                ;; sからでもuからでも持ってこられる文字のうちで、sから持ってこられる文字の数の最大数
                ;; v中の文字数で抑えられることに注意する
                (sum (c su*)
                  (min (gethash c cs 0) (gethash c cv 0))))
             n)
         (>= (+ (reduce #'+ u* :key (mapper cv))
                ;; sからでもuからでも持ってこられる文字のうちで、uから持ってこられる文字の数の最大数
                (sum (c su*)
                  (min (gethash c cu 0) (gethash c cv 0))))
             n))))
