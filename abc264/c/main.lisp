...
(defun submatch (rows cols a b)
  (loop for row in rows
        for i from 0
        always (loop for col in cols
                     for j from 0
                     always (= (aref a row col)
                               (aref b i j)))))

(defun solve (h1 w1 h2 w2 a b)
  (when (and (>= h1 h2) (>= w1 w2))
    (some (lambda (rows)
            (some (lambda (cols)
                    (submatch rows cols a b))
                  (combinations (range w1) w2)))
          (combinations (range h1) h2))))
      
(defun main ()
  (readlet (h1 w1)
    (let ((a (read-matrix h1 w1)))
      (readlet (h2 w2)
        (let ((b (read-matrix h2 w2)))
          (println (if (solve h1 w1 h2 w2 a b)
                       "Yes"
                       "No")))))))
