...
(defun times (counter d)
  (if (= d 12)
      (list (list 12))
      (ecase (gethash d counter)
        (1 (list (list d) (list (- 24 d))))
        (2 (list (list d (- 24 d)))))))
...
(defun solve (ds)
  (let ((c (counter ds)))
    (if (or (gethash 0 c)
            (some (compose #>=3 (mapper c 0))
                  (range 1 12))
            (>= (gethash 12 c 0) 2))
        0
        (maximize (times (forks (ht-keys c) (curry #'times c)))
          (reduce #'min
                  (map-adjacents #'time-diff
                                 (nconc (list 0)
                                        (sort (flatten times) #'<)
                                        (list 24))))))))
