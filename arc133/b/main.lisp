...
(defun longest-monotonic-subarray (sequence &key key (test #'<))
...
(defun solve (n ps qs)
  (let* ((p->pos (arr ps (range n) (1+ n)))
         (q->pos (arr qs (range n) (1+ n)))
         (pairs (collecting* ((p ps) (kp p (1+ n) p))
                  (cons (aref p->pos p)
                        (aref q->pos kp)))))
    (longest-monotonic-subarray (sort pairs #'pair<)
                                :key #'cdr)))
