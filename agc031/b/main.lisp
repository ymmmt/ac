...
(defun alist (keys data &optional alist)
  (nconc (nreverse (cl:pairlis keys data))
         alist))
...
(defun solve (n c)
  (let ((jss (vector-accum-ht (alist (coerce-list c)
                                      (range n)))))
    (with-memos ((dp (i)
                   (if (zerop i)
                       1
                       (let* ((js (gethash (aref c i) jss))
                              (pos (1- (bisect-left js i)))
                              (j (when (>= pos 0) (aref js pos))))
                         (if (or (= pos -1) (= (1+ j) i))
                             (dp (1- i))
                             (md (+ (dp (1- i))
                                    (dp j))))))))
...
(defun main ()
  (readlet (n)
    (let ((c (coerce-vector (collect n (read-fixnum)))))
      (println (solve n c)))))
