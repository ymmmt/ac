...
(defconstant n-vertices (* 4 (expt 10 5)))

;; https://atcoder.jp/contests/arc111/editorial/519
;; カードに書かれた数を頂点、表と裏の関係を辺とした
;; グラフを考える。このグラフは多重辺を含む可能性が
;; あるので、頂点ごとに次数を求めておいて
;; (連結成分に含まれている頂点の次数の総和)/2
;; とすることで、連結成分に含まれる辺の数を求める
;; 必要がある。
(defun solve (es)
  (let ((g (make-graph n-vertices es))
        (in (counter (mapcar #'car es)))
        (out (counter (mapcar #'cdr es))))
    (labels ((degree (v)
               (+ (gethash v in 0)
                  (gethash v out 0))))
      (reduce #'+ (dff g)
