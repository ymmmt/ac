...
;; 求める答えは、各aiを一様ランダムに[1,n]から
;; 選んだ場合の美の期待値。
;; m=2の場合を考える。(ai, ai+1)の選び方は
;; n^2通りあり、d > 0のとき、そのうち美しい
;; 選び方は(ai, ai + d)なる選び方がn - d通り、
;; (ai, ai - d) なる選び方がn - d通りの
;; 合わせて2(n - d)通り。d = 0のときはn通り。
;; よって(ai, ai+1)の選び方の美しさの期待値は
;; (/ (* (if (zerop d) 1 2) (- n d)) (expt n 2)) となる。
;; mが3以上の場合には期待値の線形性が使えて、
;; 隣り合うm - 1個の組それぞれの期待値を足し合わせたものが
;; 答えになる。
(defun solve (n m d)
  (let ((f (if (zerop d) 1 2)))
    (df (/ (* (- m 1) f (- n d))
           (expt n 2)))))

(defun main ()
  (readlet (n m d)
    (format t "~,9f~%" (solve n m d))))
